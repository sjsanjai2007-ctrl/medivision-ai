...
// Live OpenStreetMap + Overpass API preview
// ============================================================
  ...
  useEffect(() => {
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          listHospitalsApi({ lat: pos.coords.latitude, lng: pos.coords.longitude })
            .then((res) => setHospitals(res.slice(0, 2)))
            .catch(() => {});
        },
        () => {
          listHospitalsApi()
  ...
  return (
    <section aria-labelledby="nearby-hospitals-title">
      <div className="flex items-center justify-between mb-3">
        <h2 id="nearby-hospitals-title" className="text-base font-bold flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
          Nearby Hospitals
        ...
        <Link href="/hospitals">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Find nearby hospitals on OpenStreetMap</p>
              <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Live OpenStreetMap & Overpass API location search</p>
            </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-3 text-xs" style={{ color: 'var(--text-muted)' }}>
                    <span className="flex items-center gap-1">
                      <MapPin className="w-3 h-3 text-sky-500" />{formatDistance(h.distance)}
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock className="w-3 h-3 text-sky-500" />{formatTravelTime(h.travelTime)}
                    </span>
                  </div>
                </div>
              </Link>
